...
class OrderBookBase:
    def __init__(
        self, bids: Sequence[PriceLevel] = [], asks: Sequence[PriceLevel] = []
    ):
    ...
    @staticmethod
    def matching_engine(price_levels: Sequence[PriceLevel], limit_order: LimitOrder):
        ...
        for i, price_level in enumerate(price_levels):
            if (sign * price_level.price < sign * limit_order.price) or (
                remain_amount == 0
            ):
                break
    ...
    def set_limit_order(self, limit_order: LimitOrder):
        ...
        if remain_amount > 0:
            if remain_amount != old_amount:
                ...
                index = bisect_left(
                    price_levels, sign * limit_order.price, key=lambda x: sign * x.price
                )
...
class OrderBookPrep(OrderBookBase):
    def track_diff_side(self, update, side: int):
        ...
        for price_level in update:
            ...
            index = bisect_left(
                price_levels, sign * price, key=lambda x: sign * x.price
            )
            if index == len(price_levels):
                if amount > 0:
                    price_levels.append(
...
class OrderBook(OrderBookBase):
    def update_limit_orders(self, updates, side):
        ...
        for update in updates:  # THINK ABOUT ORDER OF PRICE CHANGES
            index = bisect_left(
                price_levels, sign * update[0], key=lambda x: sign * x.price
            )
            if index == len(price_levels):
                if update[1] > 0:
