...
#         self.bids = SortedDict(lambda x: -x, bids)
#         self.asks = SortedDict(asks)
#         self.top_n = top_n
#         del self.bids.keys()[self.top_n:]
#         del self.asks.keys()[self.top_n:]
...
#         for update in updates:
#             if update[1] > 0:
#                 self.bids[update[0]] = update[1]
#             else:
#                 self.bids.pop(update[0], None)
...
#         for update in updates:
#             if update[1] > 0:
#                 self.asks[update[0]] = update[1]
#             else:
#                 self.asks.pop(update[0], None)
#         del self.asks.keys()[self.top_n:]
...
#     def set_bid_limit_order(self, limit_order: Tuple[int, int, int]):
#         if limit_order[0] in self.bids:
#             self.bids[limit_order[0]] += limit_order[1]
#         else:
#             self.bids[limit_order[0]] = limit_order[1]
#         # index = bisect_left(self.bids, -limit_order[0], key=lambda x: -x[0])
#         # if index == len(self.bids):
...
#     def set_ask_limit_order(self, limit_order: Tuple[int, int, int]):
#         if limit_order[0] in self.asks:
#             self.asks[limit_order[0]] += limit_order[1]
#         else:
#             self.asks[limit_order[0]] = limit_order[1]
#         # index = bisect_left(self.asks, limit_order[0], key=lambda x: x[0])
#         # if index == len(self.asks):
...
#         # i = 0
#         # keys = self.asks.keys()
#         # while i < len(keys) and self.asks[keys[i]][1] <= market_order[0]:
#         #     market_order[0] -= self.asks[keys[i]][1]
#         #     i += 1

#         # if i == len(keys):
#         #     self.asks = SortedDict()
#         # else:
#         #     self.asks[keys[i]][1] -= market_order[0]
#         #     del keys[:i]
#         order_size = market_order[0]
#         for k, v in self.asks.items():
#             if v <= order_size:
#                 order_size -= v
#                 self.asks.pop(k)
#             else:
#                 self.asks[k] -= order_size
#                 break
...
#         # i = 0
#         # keys = self.bids.keys()
#         # while i < len(keys) and self.bids[keys[i]][1] <= market_order[0]:
#         #     market_order[0] -= self.bids[keys[i]][1]
#         #     i += 1

#         # if i == len(keys):
#         #     self.bids = SortedDict(lambda x: -x)
#         # else:
#         #     self.bids[keys[i]][1] -= market_order[0]
#         #     del keys[:i]
#         order_size = market_order[0]
